...
export class LoginComponent implements OnInit{
  ...
  ngOnInit() {
    // this._serverevent.getServerSentEvent('http://3.88.84.235:3000/progress').subscribe(data => console.log(data))
  }
  ...
  login(){
    ...
    this.http.post<LoginResponse>("http://3.88.84.235:3000/api/employees/login",bodyData).subscribe((resultData:any)=>{
      ...
      if(resultData.successMsg == "User Logged in Successfully"){
        ...
        // this._serverevent.getServerSentEvent('http://3.88.84.235:3000/progress').subscribe(data => console.log(JSON.parse(data.data).msg))
